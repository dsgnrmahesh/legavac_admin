...
export default function AddCompany({ setSmShow, editID }) {
  ...
  async function handlechangelogoimage(e) {
    setState({
      ...state,
      [e.target.name]: getFileName(state.CompanyName, e.target.files[0]),
    });
    ...
    formData.append(
      "file",
      e.target.files[0],
      getFileName(state.CompanyName, e.target.files[0])
    );
    await UploadLogoImage(formData, configlogoimage)
      .then((response) => {
  ...
  return (
    <>
      ...
      {progresslogoimage && (
        <ProgressBar now={progresslogoimage} label={`${progresslogoimage}%`} />
      )}
